import { Truck, Plus, Search, X, Pencil, CalendarDays, Wallet, LogOut, ListChecks, FileWarning, ChevronLeft, ChevronRight, Rows3, Download, PanelRightOpen, RefreshCw, SlidersHorizontal, Sparkles, CheckCircle2, Save, Trash2, ArrowLeft } from "lucide-react";
...
export default function AracDurumlari() {
    ...
    const [page,setPage]=useState(1),[pageSize,setPageSize]=useState(()=>Number(localStorage.getItem("fts_vehicle_status_page_size"))||25),[compact,setCompact]=useState(()=>localStorage.getItem("fts_vehicle_status_density")==="compact");
    ...
    useEffect(() => { loadData(); }, []);
    useEffect(() => { localStorage.setItem("fts_vehicle_status_page_size", String(pageSize)); }, [pageSize]);
    useEffect(() => { localStorage.setItem("fts_vehicle_status_density", compact ? "compact" : "comfortable"); }, [compact]);
    ...
    useEffect(()=>{const key=e=>{if(e.key==='Escape'&&!formOpen&&!izinModalRow&&!kesintiModalRow&&!cikisModalRow&&!listModalOpen)setSelectedRow(null);if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName)){e.preventDefault();document.querySelector('.fleet-search-input')?.focus();}if(e.altKey&&e.key.toLowerCase()==='r'){e.preventDefault();loadData();}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key);},[formOpen,izinModalRow,kesintiModalRow,cikisModalRow,listModalOpen]);
    const overviewItems = [
        {label:'Aktif araç', count:activeRows.length, icon:Truck, filter:'Tümü', tone:'indigo'},
        {label:'Müsait', count:activeRows.filter(r=>r.durum==='Müsait').length, icon:CheckCircle2, filter:'Müsait', tone:'green'},
        {label:'Seferde', count:activeRows.filter(r=>r.durum==='Seferde').length, icon:Truck, filter:'Seferde', tone:'blue'},
        {label:'Evrak takibi gereken', count:activeRows.filter(r=>r.documentRisk!=='ok').length, icon:FileWarning, filter:'problem', tone:'amber'},
    ];
    const activeFilterCount = (search?1:0)+(statusFilter!=='Tümü'?1:0)+(bolgeFilter!=='Tümü'?1:0)+(aracTipFilter!=='Tümü'?1:0)+(onlyProblematic?1:0);
    const clearFilters = ()=>{setSearch('');setStatusFilter('Tümü');setBolgeFilter('Tümü');setAracTipFilter('Tümü');setOnlyProblematic(false);};
    return <div className={`arac-page fleet-modern ${compact?'fleet-compact':''}`}>
        <div className="arac-hero">
            ...
            <div className="hero-actions">
                <button className="hero-refresh-btn" title="Verileri yenile (Alt+R)" onClick={loadData}><RefreshCw size={16} className={loading ? "spin" : ""}/> Yenile</button>
                <button className="add-btn premium-add" onClick={openAddForm}><span className="button-glow"/><Plus size={16}/> Araç Ekle</button>
        ...
        <div className="fleet-overview">{overviewItems.map(({label,count,icon:Icon,filter,tone})=><button type="button" className={`fleet-kpi tone-${tone} ${filter==='problem' ? (onlyProblematic?'active':'') : (statusFilter===filter?'active':'')}`} key={label} onClick={()=>{if(filter==='problem'){setOnlyProblematic(v=>!v);setStatusFilter('Tümü');}else{setOnlyProblematic(false);setStatusFilter(statusFilter===filter&&filter!=='Tümü'?'Tümü':filter);}}}><span className="fleet-kpi-icon"><Icon size={21}/></span><div><small>{label}</small><strong>{count}</strong><em>{filter==='Tümü'?'Tüm aktif kayıtlar':'Filtrelemek için tıklayın'}</em></div></button>)}</div>
        <div className="filter-card">
            <div className="search-box"><Search size={17}/><input className="fleet-search-input" aria-label="Araçlarda ara" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Plaka, sürücü, telefon, tedarikçi ara..." /><kbd>/</kbd></div>
            ...
            <label className="fleet-filter-label"><span>Araç tipi</span><select aria-label="Araç tipi filtresi" value={aracTipFilter} onChange={(e) => setAracTipFilter(e.target.value)}>{aracTipOptions.map((x) => <option key={x}>{x}</option>)}</select></label><button className="fleet-clear" disabled={!activeFilterCount} onClick={clearFilters}><X size={14}/> Temizle{activeFilterCount>0&&<b>{activeFilterCount}</b>}</button>
        </div>
        <div className="fleet-utility-strip">
            <div><Sparkles size={15}/><span><strong>{filteredRows.length}</strong> araç görünümde</span>{activeFilterCount>0&&<span className="active-filter-note"><SlidersHorizontal size={13}/>{activeFilterCount} aktif filtre</span>}</div>
            <small>Kısayollar: <kbd>/</kbd> ara · <kbd>Alt+R</kbd> yenile · <kbd>Esc</kbd> paneli kapat</small>
        </div>
...
function VehicleForm({ editingRow, form, updateForm, onSubmit, onClose }) {
    return <div className="vehicle-modal">
        <form className="vehicle-form" onSubmit={onSubmit}>
            ...
            <div className="vehicle-form-actions"><button type="button" className="cancel-btn" onClick={onClose}><ArrowLeft size={15}/> Vazgeç</button><button type="submit" className="save-btn"><Save size={15}/> Kaydet</button></div>
        </form>
    </div>;
}
...
function SmallRecordModal({ title, subtitle, type, form, setForm, records = [], row, onRemove, onSubmit, onClose, leaveStatusOptions = LEAVE_STATUS_OPTIONS }) {
    ...
    return <div className="vehicle-modal">
        <form className={`small-record-modal small-record-${type}`} onSubmit={onSubmit}>
            ...
            <div className="small-modal-actions"><button type="button" className="cancel-btn" onClick={onClose}><ArrowLeft size={15}/> Vazgeç</button><button type="submit" className="save-btn"><Save size={15}/> Kaydet</button></div>
        </form>
    </div>;
}
...
function RecordMiniList({ type, records, row, onRemove }) {
    ...
    return <div className={`modal-record-list ${type}`}>
        ...
        {records.length === 0 ? <div className="modal-record-empty">Henüz kayıt yok.</div> : records.map((item) => <div className="modal-record-item" key={item.id}>
            ...
            <button type="button" onClick={() => onRemove(row, item.id)}><Trash2 size={13}/> Sil</button>
        </div>)}
    </div>;
}
