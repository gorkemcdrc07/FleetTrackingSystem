...
import "./EbebekYakitHakedis.css";
import ExcelJS from "exceljs";
import * as XLSX from "xlsx";
import { saveAs } from "file-saver";
/* ------------------------ Tema ------------------------ */
const DARK = {
    pageBg: "var(--eb-page-bg)",
    surface: "var(--eb-surface)",
    surface2: "var(--eb-surface-2)",
    border: "var(--eb-border)",
    text: "var(--eb-text)",
    textMuted: "var(--eb-text-muted)",
    zebra: "var(--eb-zebra)",
...
const downloadXlsxTemplate = async (fileName, sheetName, headers, sampleRows) => {
    ...
    applyHakedisWorkbookBranding(workbook, { title: "Ebebek Yakıt Hakediş Raporu" });
    ...
    return (
        <Box sx={{ mt: 3 }}>
            ...
            {currentStep === MAX_STEP && kmData?.hesaplanmayanSeferler?.length > 0 && (
                <Alert severity="warning" sx={{ mt: 3, bgcolor: withAlpha(DARK.red, 0.1), color: DARK.text }}>
...
export default function EbebekYakitHakedis() {
    ...
    return (
        <Box className="ebebek-hakedis-page ebebek-control-center fuelcc-page">
            <section className="fuelcc-hero">
                <div className="fuelcc-hero-copy">
                    <div className="fuelcc-eyebrow">
                        <span className="fuelcc-live-dot" />
                        HAKEDİŞLER · EBEBEK YAKIT HAKEDİŞ
                    </div>
                    <h1>Yakıt Hakediş Control Center</h1>
                    <p>Yakıt ve sefer verilerini aynı çalışma alanında yükleyin; araç bazlı SFR / BOS yakma oranlarıyla tüketim ve hakediş sonuçlarını hesaplayın.</p>
            ...
            <section className="fuelcc-workspace">
                ...
                <aside className="fuelcc-side-panel">
                    ...
                    <div className="fuelcc-rule-box"><span>HESAPLAMA KURALI</span><p>SFR / BOS yakma oranları Araç Cari & Fiyat kayıtlarından alınır ve plaka bazında uygulanır.</p></div>
