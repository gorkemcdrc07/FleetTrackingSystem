...
function EmptyState({ onCreate, createDisabled }) {
    return (
        <Stack alignItems="center" justifyContent="center" sx={{ height: "52vh", py: 6 }} spacing={2}>
            <CloseIcon sx={{ fontSize: 56, opacity: 0.6 }} />
...
/* ---------- Component ---------- */
export default function TedarikciMasraf({ onNavigate }) {
    const theme = useTheme();
    const downSm = useMediaQuery(theme.breakpoints.down("sm"));
    dayjs.locale("tr");

    let sessionUser = {};
    try { sessionUser = JSON.parse(localStorage.getItem("fts_user") || "null") || {}; } catch { sessionUser = {}; }
    const kullanici = sessionUser?.kullanici || sessionUser?.ad || "";
    const kullaniciRol = sessionUser?.rol || "";
    ...
    const showToast = useCallback((severity, message) => setNotice({ open: true, message, severity }), []);
    ...
    const loadPermissions = useCallback(async () => {
        try {
            setPermLoading(true);
            let current = {};
            try { current = JSON.parse(localStorage.getItem("fts_user") || "null") || {}; } catch { current = {}; }
            const role = String(current?.rol || "").toLocaleUpperCase("tr-TR");
            const isAdmin = ["ADMIN", "YONETICI", "YÖNETİCİ"].includes(role);
            const yetki = Array.isArray(current?.yetki) ? current.yetki : [];
            const row = yetki.find((x) => ["Tedarikçi Masraf", "Tedarikci Masraf"].includes(x?.page));
            const actions = Array.isArray(row?.actions) ? row.actions : [];
            setPerms({
                canCreate: isAdmin || actions.includes("create"),
                canEdit: isAdmin || actions.includes("update"),
                canDelete: isAdmin || actions.includes("delete"),
                canApprove: isAdmin || actions.includes("update"),
            });
        } catch (err) {
            console.error("FTS permission load error:", err);
    ...
    const handleOpenYeni = () => {
        if (!perms.canCreate) {
            showToast("warning", "Yeni ekleme yetkiniz yok.");
            return;
        }
        setForm(BOS_FORM);
        setDuzenlemeId(null);
    ...
    const handleSubmit = async (e) => {
        ...
        if (duzenlemeId && !perms.canEdit) {
            showToast("warning", "Düzenleme yetkiniz yok.");
            return;
        }

        const bedelParsed = parseFloat(String(form.bedel).replace(",", "."));
        if (Number.isNaN(bedelParsed)) {
            showToast("error", "Geçerli bir bedel giriniz.");
            return;
        }
        const kayit = {
            ...form,
        ...
        if (duzenlemeId) {
            ...
            try {
                const bekir = await supabase.from("kullanicilar").select("id").eq("kullanici", "BEKİR AKCAGÖZ").single();
                if (bekir.data?.id) {
        ...
        if (!sonuc.error) {
            handleCloseForm();
            await veriGetir();
            showToast("success", "✅ Masraf kaydedildi.");
        } else {
            showToast("error", "❌ Masraf kaydedilemedi.");
        }
    };
    ...
    const handleSil = async () => {
        ...
        if (!perms.canDelete) {
            showToast("error", "❌ Silme yetkiniz yok.");
        ...
        if (!error) {
            await veriGetir();
            showToast("info", "🗑️ Masraf silindi.");
        } else {
    ...
    const handleOnayla = async (id) => {
        if (!perms.canApprove) {
            showToast("warning", "Onaylama yetkiniz yok.");
            return;
        }
        const { error } = await supabase.from("tedarikci_masraflar").update({ statu: "ONAYLANDI" }).eq("id", id);
        if (!error) {
            await veriGetir();
            showToast("success", "✔️ Masraf onaylandı.");
        } else {
            showToast("error", "❌ Onaylama sırasında hata oluştu.");
        }
    };
    ...
    const handleReelKaydet = async (id) => {
        if (!reelDurum[id]) {
            showToast("warning", "Önce kutucuğu işaretleyin.");
            return;
        }
        ...
        if (!error) {
            ...
            await veriGetir();
            showToast("success", "📌 REEL’e işlendi olarak kaydedildi.");
        } else {
    ...
    const exportToExcel = async () => {
        ...
        applyHakedisWorkbookBranding(workbook, { title: "Tedarikçi Masraf Raporu" });
    ...
    return (
        <Box
            className="hakedis-premium-page tedarikci-masraf-premium-page"
        ...
        >
                ...
                >
                    ...
                    >
                        ...
                        <Stack direction="row" spacing={1.25} alignItems="center">
                            ...
                            {/* 🧭 Geri / Anasayfa / Yeni */}
                            <Button size="small" variant="outlined" onClick={() => onNavigate?.("Dashboard")} title="Geri">
                                Geri
                            </Button>
                            <Button size="small" variant="text" startIcon={<HomeIcon />} onClick={() => onNavigate?.("Dashboard")}>
                                Anasayfa
                            </Button>
            ...
            <Snackbar open={notice.open} autoHideDuration={4000} onClose={() => setNotice((n) => ({ ...n, open: false }))} anchorOrigin={{ vertical: "bottom", horizontal: "right" }}>
