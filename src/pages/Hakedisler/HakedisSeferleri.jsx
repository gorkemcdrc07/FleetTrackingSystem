...
// TMS URL sabitleri — Vite dev/prod aynı göreli API yollarını kullanır.
// Geliştirmede vite.config.ts bu iki yolu uzak FTS backend'ine proxy'ler.
const TMS_LOGIN_URL = "/api/reel-auth/login";
const TMS_ADD_EXPENSE_URL = "/api/tmsdespatchincomeexpenses/addexpense";
...
/* ---------------------- FTS YETKİLERİ ---------------------- */
function getFtsSession() {
    try { return JSON.parse(localStorage.getItem("fts_user") || "null") || {}; } catch { return {}; }
}
function getFtsActions(pageAliases = []) {
    const user = getFtsSession();
    const role = String(user?.rol || "").toLocaleUpperCase("tr-TR");
    if (["ADMIN", "YONETICI", "YÖNETİCİ"].includes(role)) return ["view","create","update","delete","export"];
    const yetki = Array.isArray(user?.yetki) ? user.yetki : [];
    const row = yetki.find((x) => pageAliases.includes(x?.page));
    return Array.isArray(row?.actions) ? row.actions : [];
}
async function loadUploadPermission() {
    const actions = getFtsActions(["Sefer Kira & Sürücü Hakediş", "Hakediş Seferleri"]);
    return { canUpload: actions.includes("create") || actions.includes("update") || actions.includes("view") };
}

/* ---------------------- ANA BİLEŞEN ---------------------- */
export default function HakedisSeferleri({ onFileReady, onNavigate }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                setPermLoading(true);
                const p = await loadUploadPermission();
                setPerms(p);
            } catch (e) {
    ...
    const handleDownloadTemplate = async () => {
        ...
        applyHakedisWorkbookBranding(workbook, { title: "Hakediş Seferleri Raporu" });
    ...
    return (
        <Box
            className="hakedis-premium-page hakedis-seferleri-premium-page"
            sx={{
                minHeight: "100dvh",
                py: 4,
                px: { xs: 1.5, md: 2.5 },
                ...
                >
                    ...
                    >
                        ...
                        <Stack direction="row" spacing={1} flexShrink={0}>
                            <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => onNavigate?.("Dashboard")}>
                                Geri
                            </Button>
                            <Button variant="outlined" startIcon={<HomeIcon />} onClick={() => onNavigate?.("Dashboard")}>
                                Anasayfa
                            </Button>
                        </Stack>
                    </Stack>
